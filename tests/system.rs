...
fn test_add_license() {
    ...
    let license_src = "test_license_src.txt";
    let license_target_dir = "test_license_target_dir";
    let license_target = Path::new(license_target_dir).join("copyright");

    std::fs::write(license_src, "MIT License Content").unwrap();

    let brigid = Brigid::new(root)
        .add_license(license_src, &license_target)
        .establish()
        .expect("Failed to establish");

    assert!(license_target.exists());
    assert_eq!(std::fs::read_to_string(license_target).unwrap(), "MIT License Content");
    assert!(brigid.no_warnings());

    brigid.delete_all().expect("Failed to delete");
    std::fs::remove_file(license_src).unwrap();
...
fn test_license_not_found_warning() {
    ...
    let brigid = Brigid::new(root)
        .add_license("non_existent_license.txt", "/tmp/non_existent_target")
        .establish()
        .expect("Failed to establish");

    assert!(brigid.has_warnings());
    let warnings = brigid.get_warnings();
    assert!(warnings.iter().any(|w| w.to_string().contains("License source not found")));
...
fn test_priority_out_of_bounds() {
    ...
    assert!(brigid_high.get_warnings().iter().any(|w| w.to_string().contains("Priority too high")));
    ...
    assert!(brigid_low.get_warnings().iter().any(|w| w.to_string().contains("Priority too low")));
